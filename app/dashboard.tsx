...
export default function DashboardScreen() {
  ...
  useEffect(() => {
      // 1. Περιμένουμε το Auth (για να δουλεύει και Offline η ταυτοποίηση)
      const unsubscribeAuth = onAuthStateChanged(auth, (user) => {
          if (user) {
              
              // 2. REALTIME LISTENER ΓΙΑ ΤΟ ΟΝΟΜΑ
              // Συνδεόμαστε στο έγγραφο του χρήστη και ακούμε για αλλαγές.
              // Το { includeMetadataChanges: true } είναι απαραίτητο για να δουλεύει offline.
              const userRef = doc(db, "users", user.uid);
              
              const unsubscribeUser = onSnapshot(userRef, { includeMetadataChanges: true }, (docSnap) => {
                  if (docSnap.exists()) {
              ...
              // Καθαρισμός του listener όταν αλλάζει ο χρήστης
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        <View>
            ...
            {/* Τώρα το userName ανανεώνεται αυτόματα */}
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <TouchableOpacity style={[styles.bigCard, styles.listCard]} onPress={() => router.push('/teams/my-teams')}>
          ...
          <View>
            ...
            <Text style={[styles.cardSubtitle, { color: '#475569' }]}>Δείτε τα έργα σας.</Text>
          </View>
          <Ionicons name="chevron-forward" size={24} color="#cbd5e1" style={{marginLeft: 'auto'}} />
        </TouchableOpacity>

        {/* 2. ΚΟΥΜΠΙ: ΔΗΜΙΟΥΡΓΙΑ ΟΜΑΔΑΣ */}
        ...
        {/* 3. ΚΟΥΜΠΙ LINK: ΕΧΩ ΚΩΔΙΚΟ (ΔΙΑΚΡΙΤΙΚΟ) */}
